...
void OnlineBookCatalog::updateBookCatalog(const JsonBookData& jsonBookData){
	...
	for(rapidjson::SizeType i=0; i<books.Size(); i++){
		std::shared_ptr<Book> book =
			std::make_shared<Book>(books[i]["id"].GetString(),
								   books[i]["author"].GetString(),
								   books[i]["title"].GetString(),
								   books[i]["genre"].GetString(),
								   books[i]["price"].GetString(),
								   books[i]["publish_date"].GetString(),
								   books[i]["description"].GetString());
